...
(function (bodyScrollLock, MicroModal, Rellax, themeCurrency, Sqrl, themeAddresses, axios, Flickity, FlickityFade, ellipsed, FlickitySync, Poppy, AOS) {
  ...
  class CartItems {
    constructor(section) {
      ...
      this.subtotal = this.container.querySelector(selectors$t.subtotal);
    ...
    init() {
      return window
        ...
        .then((response) => {
          ...
          if (this.cartMessage.length > 0) {
            this.shippingAmount = response.total_price;
            this.updateProgress();
          }
          return response;
        })
  ...
  class ProductAddButton {
    constructor(button) {
      this.button = button;
      this.section = button.closest(selectors$e.section);
      this.drawer = this.section.querySelector(selectors$e.addDrawerWrapper);
      this.source = this.section.querySelector(selectors$e.addDrawerTemplate).innerHTML;
      this.form = null;
    ...
    addSuccess(variant) {
      ...
      const pageCartAjax = document.querySelector('[data-ajax-disable="false"]');
      if (cartDrawer) {
      ...
      } else if (pageCartAjax) {
        pageCartAjax.dispatchEvent(new CustomEvent('theme:cart:reload', {bubbles: true}));
      }
