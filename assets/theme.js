...
theme.detectNav = function(){
  var navWidthLeft = $('.nav--desktop .left-menu').outerWidth();
  var navWidthRight = $('.nav--desktop .right-menu').outerWidth();
  var logoWidth = $('.logo-wrapper').outerWidth();
  var widthLimit = $('.site-header .wrapper').width();
  var totalWidth = navWidthLeft + navWidthRight + logoWidth;
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    initAdd: function() {
      ...
      var ajaxAddItem = function(data) {
        $.ajax({
          ...
          error: addError,
          success: addSuccess,
          complete: [updateHeaderTotal, updateCartTotal]
        });
      }
      ...
      var addSuccess = function(product){
        ...
        popdown();
        $('.product-options select').val('').trigger('change')
      }
    ...
    updateAllVariants: function(evt) {
      ...
      if (variant) {
        ...
        var form = jQuery(this.selectors.productSection).find('form');
        ...
        if (variant) {
          $('.variant-message').hide();        
          $('.variant-message-' + variant.id).show();        
        }

        var variant_price = variant.price;
        var compare_at_price = variant.compare_at_price;
        if ($('.variant-message-' + variant.id).data('discounted-price')) {
          var compare_at_price = variant.price;
          var variant_price = parseInt($('.variant-message-' + variant.id).data('discounted-price'), 10);
        }
        
        $(this.selectors.productPrice).html("<span class='money'>" + slate.Currency.formatMoney(variant_price, moneyFormat) + "</span>");

        
        // Also update and show the product's compare price if necessary
        if (compare_at_price > variant_price) {
          $(this.selectors.comparePrice).html("<span class='money'>" + slate.Currency.formatMoney(compare_at_price, moneyFormat) + "</span>");
  ...
  Cart.prototype = _.assignIn({}, Cart.prototype, {
    ...
    initQuantity: function() {
      ...
      $('.js-qty__adjust').off().on('click', function() {
        ...
        debugger
        if (false && $element.hasClass('js-qty__adjust--plus')){
        ...
        complete: function(jqxhr, text) {
          ...
          $.getJSON('/cart.js')
           .done(function(cart) {
             var newTotal = slate.Currency.formatMoney(cart.total_price, theme.moneyFormat);
             $('.cartCost').html("(<span class='money'>" + newTotal + "</span>)").removeClass('hidden-count');
             // Below added to dynamically update cart icon total
             var cartItemCounter = document.querySelectorAll('span.cart-item-count');
             var i;
             for (i = 0; i < cartItemCounter.length; i++) {
               cartItemCounter[i].innerText = cart.item_count;
             }  
          });
        }
      }
...
$(document).ready(function() {
  ...
  var $productOptionsMessage = $('.product-options-message');
  var checkOptions = function() {    
    var $addToCart = $('.addToCart');
    if ($productOptions.length) {
      var max = $productOptions.data('max');
      var total = 0;
      ...
      if (total != max) {
        ...
        if (max < total) {
          $productOptionsMessage.text('You can only select up to ' + max + '. Remove ' + (total - max) + '.');          
        } else {
          $productOptionsMessage.text('Please select up to ' + max + ' to continue. Add ' + (max - total) + ' more.');                    
        }
      } else {
        $addToCart.attr('disabled', false);
        $addToCart.removeClass('disabled');  
        $productOptionsMessage.text('');                            
      }
    }
    
  }
  checkOptions();
  $productOptions.find('select').on('change', checkOptions);
